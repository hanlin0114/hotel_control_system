...
session_start();
(integer)$u_id=$_SESSION['u_id'];
$page=$_POST['pageNum'];
$conn=new db_control();
$conn->db_connect();
$conn->select_db($tableName="bill_info","*",$where="b_customer_id=$u_id and b_status=1" );
...
$query=mysqli_query($c_conn,"select * from bill_info where b_customer_id=$u_id and b_status=1 order by b_id asc limit $startPage,$pageSize");
