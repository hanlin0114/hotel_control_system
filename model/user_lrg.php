...
class user_lgr{
	...
	public function a_register($email,$username,$passwd,$sex,$u_level){//针对创建管理员级的用户
		$passwd=md5($passwd);
		$db_conn=new db_control();//数据库连接
		$db_conn->db_connect();
		$db_conn->select_db($tableName,"*","email='$email'");
		$result=$db_conn->return_result();
		$obj=mysqli_fetch_object($result);
		if(!isset($obj)){
			return -1;//表示邮箱已经被注册
			...
			$col=array('email'=>$email,'username'=>$username,'password'=>$passwd,'sex'=>$sex,'u_level'=>$u_level);
			$db_conn->insert_db($tableName,$col);
			$result=$db_conn->return_result();
			if($result)
				return 1;//返回1表示操作成功
			else 
				return 0;//返回0表示数据库操作出现问题
		}
	}
}
